...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.gameinfo = this.node.getChildByName("gameinfo");
    ...
    initView:function(){
        ...
        this._mjcount.string = "" + cc.vv.gameNetMgr.shenyupai;
        this._gamecount = this.node.getChildByName("roundbg").getChildByName('gamecount').getComponent(cc.Label);
        ...
        this._magicPai.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",cc.vv.gameNetMgr.magicPai, null, true);
        ...
        this.myholdsNode = myselfChild.getChildByName("holds");
        this.myholdsList = this.myholdsNode.getChildByName("list");
    ...
    initPaiQiangView:function(){
        var gameChild = this.node.getChildByName("game");
        this.wallMainNode = gameChild.getChildByName("wall");
        // myself right up left
        this.paiQianNode = [];

        this.initSidePaiQiang("myWall");
        this.initSidePaiQiang("rightWall");
        this.initSidePaiQiang("upWall");
        this.initSidePaiQiang("leftWall");
    },

    initSidePaiQiang:function(nodename){
        var wallNode;
        var listWall;
        var onePai;
        var name;

        wallNode = this.wallMainNode.getChildByName(nodename);
        listWall = [];
        for (let index = 0; index < 34; index++) {
            ...
            name = name + (Math.floor(index / 2 + 1) >= 10? Math.floor(index / 2 + 1) : "0" + Math.floor(index / 2 + 1));
            onePai = wallNode.getChildByName(name);
            listWall.push(onePai);
        }

        this.paiQianNode.push(listWall);
    },
    ...
    initEventHandlers:function(){
        ...
        this.node.on('hupai',function(data){
            ...
            if(cc.vv.replayMgr.isReplay() == true){
                ...
                opt.sprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",data.hupai, null, true);                
            }
        ...
        this.node.on('mj_count',function(data){
            self._mjcount.string = "" + cc.vv.gameNetMgr.shenyupai;
            //要消失的牌的index
            var index = self._maxMaJiangNum - cc.vv.gameNetMgr.numOfMJ - 1;
            self.hidePaiQianOnePaiByIndex(index);
        });
        
        this.node.on('game_magic',function(data){
            cc.log3.debug("magic pai = " + cc.vv.gameNetMgr.magicPai);
            self._magicPai.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",cc.vv.gameNetMgr.magicPai, null, true);
        });
    ...
    */
    ...
    addGangpai:function(btnName,paiWik,pai,actpai){
        ...
        for(var i = 0; i < this._GangpaiOpts.childrenCount; ++i){
            ...
            if(child.name == "op" && child.active == false){
                ...
                spriteL.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",actpai[0],null,true);
                ...
                spriteC.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",actpai[1],null,true);

                var spriteR = child.getChildByName("opTarget2").children[0].getComponent(cc.Sprite);
                spriteR.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",actpai[2],null,true);

                 var spriteR = child.getChildByName("opTarget3").children[0].getComponent(cc.Sprite);
                spriteR.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",actpai[3],null,true);
    ...
    addChipai:function(btnName,paiWik,pai){
        ...
        for(var i = 0; i < this._ChipaiOpts.childrenCount; ++i){
            ...
            if(child.name == "op" && child.active == false){
                ...
                spriteL.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",paiL,null,true);
                ...
                spriteC.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",paiC,null,true);
                ...
                spriteR.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID("M_",paiR,null,true);
    ...
    onBtnRoundBg:function(params) {
        this.gameinfo.getComponent("GameInfo3D").show(cc.vv.gameNetMgr.conf);
    },
    ...
    getMJIndexspecial:function(side,index, parentNode) {
        var name = "";
        if (side == "up") {
            if ((index + 1) < 10) {
                name = "e_mj_0" + (index + 1);
            ...
            return parentNode.getChildByName(name);
        }
        return parentNode.children[this.getMJIndex(side, index)];
    },
    ...
    initMopai:function(seatIndex,pai){
        ...
        if(pai == null){
            ...
            //处理回放的时候他人的牌面
            if (side != "myself") {
                mjSprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre,pai,side,false);
            }else{
                mjSprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre,pai,null,true);
            }
            ...
            if(!cc.vv.replayMgr.isReplay()){
                ...
                var sprite = nc.getComponent(cc.Sprite);             
                sprite.spriteFrame = cc.vv.mahjongmgr.getHoldsEmptySpriteFrame(side, sprite.node.name);
            }            
        }
    ...
    initEmptySprites:function(seatIndex){
        ...
        for(var i = 0; i < holds.childrenCount; ++i){
            ...
            spriteFrame = cc.vv.mahjongmgr.getEmptySpriteFrame(side, i);
            //nc.scaleX = 1.0;
            //nc.scaleY = 1.0;
    ...
    initOtherMahjongs:function(seatData){
	    ...
	    for(var i = 0; i < num; ++i){
            var node = this.getMJIndexspecial(side, i, sideHolds);
            node.active = false;
        }
        ...
        if(holds != null && holds.length > 0){
            ...
            for(var i = 0; i < holds.length; ++i){
                ...
                var spriteNode = this.getMJIndexspecial(side, i + num, sideHolds);
                spriteNode.active = true;

                var mjSprite = spriteNode.getChildByName("MyMahJongPai").getComponent(cc.Sprite); 
                mjSprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre,holds[i],side,false);                            
            }
            
            if(holds.length + num == 13){
                var lasetIdx = this.getMJIndexspecial(side, 13, sideHolds);
                lasetIdx.active = false;
            }
        }
    },
    ...
    setSpriteFrameByMJID:function(pre,sprite,mjid){
        sprite.spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre,mjid,null,true);
    ...
    //获得牌墙开始的位置和方向
    initPaiQiangAttri:function(){
        var sum = (cc.vv.gameNetMgr.saizi_1 || 0) + (cc.vv.gameNetMgr.saizi_2 || 0);
        sum = sum + cc.vv.gameNetMgr.getLocalIndex(cc.vv.gameNetMgr.button);
        var localIndex = (sum - 1) % 4;
        this.paiQian_startSideId = localIndex;

        if(localIndex == 3){
            this.paiQian_fanPaiSideId = 0;
        }else{
            this.paiQian_fanPaiSideId = localIndex + 1;
        }

        //保存max 用于后面盖牌
        this.max = Math.max(cc.vv.gameNetMgr.saizi_1, cc.vv.gameNetMgr.saizi_2);
        var min = Math.min(cc.vv.gameNetMgr.saizi_1, cc.vv.gameNetMgr.saizi_2);
        
        //计算牌墙的起点index
        this.paiQian_startIndex = this.paiQian_startSideId * 34 + min * 2;

        //设置翻牌 白板花不翻
        if(!cc.vv.gameNetMgr.conf.bbh){
            var side = cc.vv.mahjongmgr.getSide(this.paiQian_fanPaiSideId);
            var pqindex = this.paiQianNode[this.paiQian_fanPaiSideId].length - this.max * 2;
            this.paiQianNode[this.paiQian_fanPaiSideId][pqindex].getComponent(cc.Sprite).spriteFrame = cc.vv.mahjongmgr.getPaiQiangSpriteFrameBySide(side, this.max, 1);
            this.paiQianNode[this.paiQian_fanPaiSideId][pqindex].getChildByName("mj").active = true;
            this.paiQianNode[this.paiQian_fanPaiSideId][pqindex].getChildByName("mj").getComponent(cc.Sprite).spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(null, cc.vv.gameNetMgr.magicPai, side, false);
        }
       
        this.refreshPaiQiang();
    },

    //翻的牌盖回去
    hidePaiQianFanPai:function(){
        var pqindex = this.paiQianNode[this.paiQian_fanPaiSideId].length - this.max * 2;
        var side = cc.vv.mahjongmgr.getSide(this.paiQian_fanPaiSideId);
        this.paiQianNode[this.paiQian_fanPaiSideId][pqindex].getComponent(cc.Sprite).spriteFrame = cc.vv.mahjongmgr.getPaiQiangSpriteFrameBySide(side, this.max, 2);
        this.paiQianNode[this.paiQian_fanPaiSideId][pqindex].getChildByName("mj").active = false;
    },
    
    //刷新牌墙
    refreshPaiQiang:function(){
        //显示所有牌墙
        for (let index = 0; index < this.paiQianNode.length; index++) {
            for (let num = 0; num < this.paiQianNode[index].length; num++) {
                this.paiQianNode[index][num].active = true;
            }
        }

        //隐藏牌墙 num 被拿走的牌数
        var num = 136 - cc.vv.gameNetMgr.numOfMJ;
        for (let index = 0; index < num; index++) {
            this.hidePaiQianOnePaiByIndex(index);
        }
    },

    //隐藏一张牌墙 index 指这局游戏拿走的麻将数
    hidePaiQianOnePaiByIndex:function(index){
        //排除index为NaN
        if (Number(index) >= 0) {
            index = this.paiQian_startIndex + index;
            
            var num = index % 34;
            //顺时针拿牌处理
            var sideid = this.paiQian_startSideId - (Math.floor(index / 34) - this.paiQian_startSideId);
            //保证sideid为正值
            sideid = (sideid + 4) % 4;
            //隐藏
            this.paiQianNode[sideid][num].active = false;
        }
    },
    ...
    setMyholdsPaiToShow(isshow){
        this.myholdsNode.active = isshow;
        this.myholdsList.active = isshow;
    },
