...
cc.Class({
    ...
    properties: {
        ...
        _androidDir:"com/eaugame/wtmahjong/voice",
    },
    ...
    prepare:function(filename){
        ...
        if(cc.sys.os == cc.sys.OS_ANDROID){
            jsb.reflection.callStaticMethod(this._androidDir + "/VoiceRecorder", "prepare", "(Ljava/lang/String;)V",filename);
        }
    ...
    release:function(){
        ...
        if(cc.sys.os == cc.sys.OS_ANDROID){
            jsb.reflection.callStaticMethod(this._androidDir + "/VoiceRecorder", "release", "()V");
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
    ...
    cancel:function(){
        ...
        cc.vv.audioMgr.resumeAll();
        if(cc.sys.os == cc.sys.OS_ANDROID){
            jsb.reflection.callStaticMethod(this._androidDir + "/VoiceRecorder", "cancel", "()V");
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
    ...
    clearCache:function(filename){
        if(cc.sys.isNative){
            var url = this._voiceMediaPath + filename;
            //cc.log3.debug("check file:" + url);
            if(jsb.fileUtils.isFileExist(url)){
                //cc.log3.debug("remove:" + url);
                jsb.fileUtils.removeFile(url);
            }
            if(jsb.fileUtils.isFileExist(url + ".wav")){
                //cc.log3.debug("remove:" + url + ".wav");
    ...
    play:function(filename){
        ...
        cc.vv.audioMgr.pauseAll();
        if(cc.sys.os == cc.sys.OS_ANDROID){
            jsb.reflection.callStaticMethod(this._androidDir + "/VoicePlayer", "play", "(Ljava/lang/String;)V",filename); 
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
    ...
    stop:function(){
        ...
        cc.vv.audioMgr.resumeAll();
        if(cc.sys.os == cc.sys.OS_ANDROID){
            jsb.reflection.callStaticMethod(this._androidDir + "/VoicePlayer", "stop", "()V"); 
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
    ...
    getVoiceLevel:function(maxLevel){
        ...
        if(cc.sys.os == cc.sys.OS_ANDROID){ 
            return jsb.reflection.callStaticMethod(this._androidDir + "/VoiceRecorder", "getVoiceLevel", "(I)I",maxLevel);
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
        }
        else{
    ...
    getVoiceData:function(filename){
        if(cc.sys.isNative){
            var url = this._voiceMediaPath + filename;
            cc.log3.debug("getVoiceData:" + url);
            var fileData = jsb.fileUtils.getDataFromFile(url);
            if(fileData){
                var content = encode(fileData);
                return content;
    ...
    setStorageDir:function(dir){
        ...
        if(cc.sys.os == cc.sys.OS_ANDROID){ 
            jsb.reflection.callStaticMethod(this._androidDir + "/VoiceRecorder", "setStorageDir", "(Ljava/lang/String;)V",dir);    
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
