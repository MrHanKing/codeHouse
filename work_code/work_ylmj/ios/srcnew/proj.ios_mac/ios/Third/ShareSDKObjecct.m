...
+(void)ShareSDKshare:(NSUInteger)type parameters:(NSMutableDictionary *)shareParams{
    ...
    //不调用界面
    [ShareSDK share:[ShareSDKObjecct getType:type] parameters:shareParams onStateChanged:^(SSDKResponseState state, NSDictionary *userData, SSDKContentEntity *contentEntity, NSError *error) {
        

        NSString * jsValue = [NSString stringWithFormat:@"onShareResult(%lu,%@)",(unsigned long)state,@""];
//        JSContext * jsContent = [ShareSDKObjecct.jsContext alloc] init];
//        JSValue *value = [ShareSDKObjecct.jsContext evaluateScript:jsValue];
...
+(NSUInteger)getType:(NSInteger)type{
    NSUInteger getType;
    switch (type) {
